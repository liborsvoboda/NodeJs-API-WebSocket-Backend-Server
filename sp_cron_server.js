...
const db = require('./sp_api_models/cron_db');
//const cronbackend = require('./sp_cronbackend');
...
const cronCfg = JSON.parse(fs.readFileSync(path.join('./sp_config/cron_config.json'), 'utf8'));
...
async function sendEmail(email, confirmationEmail) {
    let info = await transporter.sendMail({
        ...
        html: "<b>Reset Hesla</b><div><a href='https://www.happytohelp.eu/resetlink?email=" + email + "&confirmationEmail=" + confirmationEmail+"'>Pro reset hesla klikněte na tento odkaz. Odkaz je platný 15 minut</a></>" // html body
    });
...
async function sendEmailVerify(email, confirmationEmail) {
    let info = await transporter.sendMail({
        ...
        //text: "Hello world?", // plain text body
        html: "<div><a href='https://www.happytohelp.eu/emailVerify?email=" + email + "&confirmationEmail=" + confirmationEmail + "'>Pre overenie tvojho emailu v aplikácii HappyToHelp klikni na tento odkaz.</a></div><div>Odkaz je platný 15 minút.</div><br /><br /><div>Ďakujeme, že si s nami!</div><div>HappyToHelp tím</div>" // html body
    });
...
let payRequest = {
    ...
    body: {
        ...
        nonce: null,
        redirect_url: "https://www.happytohelp.eu/",
        signature: null
    }
...
zongji.start({
  ...
  includeSchema: {
    'sp_HappyToHelp': ['users','contribution_payments',"problem_report"]
  },
  excludeSchema: {
    'sp_HappyToHelp': ['attachments', 'contribution', 'contribution_comments', 'contribution_likes', 'contribution_rating', 'contribution_type_list', 'currency_list', 'login_history', 'phone_codes', 'sessions', 'tag_list','tag_using']
  }
});
